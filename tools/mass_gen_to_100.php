...
// We need to keep track of how many we've successfully generated in this run
$batch_generated = 0;
$missing_slugs = [];

// 1. Gather all missing slugs first
foreach ($urls as $url) {
    $url = trim($url);
    ...
    $file = "$slug.php";
    // If it doesn't exist OR it's too small (failed gen), consider it missing
    if (!file_exists($file) || filesize($file) < 500) {
        $kw = str_replace('-', ' ', $slug); // Reverse slug to keyword
        $missing_slugs[$kw] = $slug;
    }
}

echo "Found " . count($missing_slugs) . " missing pages. Starting generation from Batch $current_batch.\n";
...
while ($current_batch <= $total_batches_to_reach && $pointer < count($keys)) {
    echo "\n--- Generating Batch $current_batch ---\n";
    $batch_generated = 0;
    ...
    echo "Batch $current_batch strictly verified and completed.\n";
    $current_batch++;
}

echo "\n=============================================\n";
echo "SUCCESS! Reached Batch 100 target process.\n";
echo "Total new pages generated in this run: $generated_count\n";
echo "=============================================\n";
?>
